...
import { iTodosBoletos } from "../interfaces";
import utils from "@/ts/utils";

const props = defineProps<{
  todosBoletos: iTodosBoletos[] | null;
}>();

const emit = defineEmits(["closeModalTodosBoletos"]);
...
onMounted(async () => {
  state.todosBoletos = props.todosBoletos;
});

const cancelar = () => {
  emit("closeModalTodosBoletos");
};
...
<template>
  <v-card>
    <v-card-title class="py-3"> Todos os Boletos </v-card-title>
    <v-card-text>
      <v-data-table
